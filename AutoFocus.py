...
if __name__ == '__main__':
    ...
    #######
    cv2.createTrackbar('Sobel Threshold', cam_name, 20, 255, nothing)
    #######
    ...
    while True:
        ...
        if img0 is not None:
            ...
            S = cv2.Sobel(img0, cv2.CV_8U, 1, 1, ksize=3)
            pos = cv2.getTrackbarPos('Sobel Threshold', cam_name)
            S[np.where(S < pos)] = 0
    ...
    while True:
        ...
        if img0 is not None:
            ...
            cv2.rectangle(img1, (x1, y1), (x2, y2), 0, 2)
            S = cv2.Sobel(img0, cv2.CV_8U, 1, 1, ksize=3)
            S[np.where(S < pos)] = 0
            # print pos
            # if x1 > x2:
            ...
            #     y2 = temp
            I_ROI = S[x1:x2, y1:y2]
            # C = cv2.Canny(I_ROI, 20, 50)
            # Value = cv2.mean(I_ROI)[0]
            sum_points = cv2.countNonZero(I_ROI)
            sum_roi = cv2.sumElems(I_ROI)
            if sum_points != 0:
                Value = sum_roi[0]/sum_points
            else:
                Value = 0
            print Value
            print sum_points
            if Value > maxValue:
                maxValue = Value
            cv2.putText(img1, "current:"+str(Value), (100, 100), cv2.FONT_HERSHEY_PLAIN, 1, 255)
            cv2.putText(img1, "max:"+str(maxValue), (100, 150), cv2.FONT_HERSHEY_PLAIN, 1, 255)
            #cv2.putText(dst, "current:"+str(Value), (100, 100), cv2.FONT_HERSHEY_PLAIN, 1, 255)
            #cv2.putText(dst, "max:"+str(maxValue), (100, 150), cv2.FONT_HERSHEY_PLAIN, 1, 255)
            sys.stdout.write('\r' + str(cam_name) + " fps: " + str(delta_time)+'\n')
            sys.stdout.flush()
            cv2.imshow(window_name, img1)
            # cv2.imshow(window_name2, S)
            st = time.time()
        else:
            print '\nImg is None',
        key = chr(cv2.waitKey(5) & 255)
